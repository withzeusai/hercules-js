...
describe("useAuthCallback", () => {
  ...
  describe("React Strict Mode simulation (the bug fix)", () => {
    it("recovers after unmount/remount cycle", async () => {
      const onSync = vi.fn().mockResolvedValue(undefined);
      const onSuccess = vi.fn();

      setAuthState({ isLoading: false, isAuthenticated: true });

      // First mount then unmount (simulating Strict Mode first pass)
      ...
      // Reset mocks to track only remount behavior
      onSync.mockClear();
      onSuccess.mockClear();

      // Remount (simulating Strict Mode second mount)
      // Before the fix, mountedRef.current would be false here,
      // causing the hook to get permanently stuck
      const { result } = renderHook(() =>
        useAuthCallback({
          isBackendAuthenticated: true,
          onSync,
    ...
    it("allows sync to run again after remount (syncStarted reset)", async () => {
      const onSync = vi.fn().mockResolvedValue(undefined);
      ...
      // Let the first mount's sync start
      await waitFor(() => {
        expect(onSync).toHaveBeenCalled();
      });
      ...
      // Remount — syncStarted should have been reset so sync runs again
      const { result } = renderHook(() =>
        useAuthCallback({
          isBackendAuthenticated: true,
          onSync,
