...
// Only platform-owned system roles are system; IAM-authored and
// tenant-authored roles are custom.
function isSystemRoleFromSource(source: "system" | "iam" | "tenant"): boolean {
...
export const listMyTenants = query({
  ...
  handler: async (ctx, args): Promise<TenantSummariesPage> => {
    ...
    if (args.status === "active") {
      if (!hasActiveRootStanding) return { tenants: [] };
      return listMyActiveTenantSummaries(ctx, {
    ...
    const tenants = (
      await Promise.all(
        page.page.map(async (principal): Promise<TenantSummary | null> => {
          ...
          if (args.isRoot !== undefined && args.isRoot !== isRoot) return null;
...
async function listMyActiveTenantSummaries(
  ...
  while (tenants.length < args.limit && scanned < ACTIVE_TENANT_SCAN_CAP) {
    const remainingResults = args.limit - tenants.length;
    const remainingScan = ACTIVE_TENANT_SCAN_CAP - scanned;
    const page = await paginator(ctx.db, schema)
      .query("principals")
      .withIndex("by_auth_user_status", (q) =>
        q.eq("herculesAuthUserId", args.tokenSubject).eq("status", "active"),
      )
...
export const listTenantUsers = query({
  ...
  handler: async (ctx, args): Promise<TenantUsersPage> => {
      ...
      status === "all"
        ...
        : await paginator(ctx.db, schema)
            ...
            .withIndex("by_scope_status_type", (q) =>
              q.eq("accessScopeId", scope.accessScopeId).eq("status", status).eq("type", "user"),
            )
            .paginate({ cursor: args.cursor ?? null, numItems: limit });
    ...
    const users = (
      ...
      .filter((user) => matchesUserQuery(user, args.query));
...
export const listTenantGroups = query({
  ...
  handler: async (ctx, args): Promise<TenantGroupsPage> => {
    ...
    const page =
      status === "all"
        ...
        : await paginator(ctx.db, schema)
            .query("principals")
            .withIndex("by_scope_status_type", (q) =>
              q.eq("accessScopeId", scope.accessScopeId).eq("status", status).eq("type", "group"),
            )
            .paginate({ cursor: args.cursor ?? null, numItems: limit });
    ...
    return {
      groups: groups.filter((group) => matchesGroupQuery(group, args.query)),
      ...(page.isDone ? {} : { cursor: page.continueCursor }),
    };
  },
});
  ...
  handler: async (ctx, args): Promise<TenantRoleSummary[]> => {
    ...
    for (const role of [...scopeRoles, ...catalogRoles]) {
      ...
      roles.push({
        ...
        roleKey: role.key,
        roleName: role.name,
        isSystemRole: isSystemRoleFromSource(role.source),
...
function matchesUserQuery(user: TenantUser, query: string | undefined): boolean {
  const normalized = normalizeSearchQuery(query);
  if (!normalized) return true;
  return (
    matchesSearch(user.name, normalized) ||
    matchesSearch(user.email, normalized) ||
    matchesSearch(user.userId, normalized)
  );
}

function matchesGroupQuery(group: TenantGroup, query: string | undefined): boolean {
  const normalized = normalizeSearchQuery(query);
  if (!normalized) return true;
  return matchesSearch(group.name, normalized) || matchesSearch(group.groupId, normalized);
}
