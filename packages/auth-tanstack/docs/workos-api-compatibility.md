...
| `signOut(options?)`                                                                       | ✅                  | Clears the session and redirects to the OIDC `end_session_endpoint` when the provider advertises one (with `id_token_hint`), else to `returnTo`. With no session it skips the provider and goes straight to the post-logout target, as WorkOS does; `getSignOutUrl` still returns a URL there rather than WorkOS's `null`. |
