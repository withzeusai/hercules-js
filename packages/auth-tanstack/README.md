...
| `HERCULES_AUTH_SCOPE`                    | no       | Scopes requested at sign-in when a call passes none. Environment fallback for the `herculesAuthMiddleware({ scope })` option (the option wins). Defaults to `openid profile email offline_access`.                   |
...
Defaults you can rely on: the callback path is `/auth/callback`, the requested scope is `openid profile email offline_access`, and users are sent to `/` after a successful callback. Override these per call (see the API below).

The default scope includes `offline_access` so the provider issues a refresh token. Without one the session cannot be renewed once the access token expires, and the refresh actions (`refreshAuth`, `refreshIdToken`, …) can only hand back the current tokens. If your provider rejects `offline_access`, request a narrower scope with `herculesAuthMiddleware({ scope })` or `HERCULES_AUTH_SCOPE`.
...
| `scope`                 | Scopes requested at sign-in when a call passes none. Wins over `HERCULES_AUTH_SCOPE`; defaults to `openid profile email offline_access` so a refresh token is issued. Narrow it only for a provider that rejects `offline_access`.                                                                                                                                                                                                   |
