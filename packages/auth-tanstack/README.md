...
| `HERCULES_AUTH_SCOPE`                    | no       | Scopes requested at sign-in when a call passes none. Environment fallback for the `herculesAuthMiddleware({ scope })` option (the option wins). Defaults to `openid profile email` — see the note on refresh tokens below. |
...
The default scope omits `offline_access` because providers that don't support it reject the whole authorization request. Without it, most providers issue **no refresh token**: the session cannot be renewed once the access token expires, and the refresh actions (`refreshAuth`, `refreshIdToken`, …) can only hand back the current tokens. If your provider supports it — Hercules Auth does — request it: `herculesAuthMiddleware({ scope: "openid profile email offline_access" })` or `HERCULES_AUTH_SCOPE`.
...
| `scope`                 | Scopes requested at sign-in when a call passes none. Wins over `HERCULES_AUTH_SCOPE`; defaults to `openid profile email`. Add `offline_access` when the provider supports it so a refresh token is issued (Hercules Auth: `"openid profile email offline_access"`).                                                                                                                                                                  |
